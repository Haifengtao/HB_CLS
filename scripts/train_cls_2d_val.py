...
def main(cfg):
    if cfg.net.name == "Unet_cls":
        ...
        labeled_f_test = h5py.File(cfg.data.test_txt, 'r')
        # 标签是四个类别，这里只做二分类
        try:
            ...
            temp = np.zeros_like(labeled_f_test['label'][:])
            temp[labeled_f_test['label'][:] == cfg.net.cls_label] = 1
            test_labels = temp.copy()
            ...
            test_labels = labeled_f_test['label'][:]
            test_labels[val_labels != 1] = 0
            pass
        ...
        test_cls_dataset = data_generator.cls_generator_2D(labeled_f_test['img'],
                                                           labeled_f_test['mask'], test_labels,
                                                           rotate_degree=0,
                                                           noise_sigma=(0, 1e-9), )
        test_dataLoader = DataLoader(test_cls_dataset, batch_size=1, shuffle=False)
    else:
    ...
    for epoch in range(cfg.train.start_epoch + 1, cfg.train.epochs + 1):
        ...
        if ec_loss < temp:
            test_ec_loss, test_dice, test_acc_value = val_model(cfg, writer, epoch, model, test_dataLoader, use_cuda)
            writer.add_scalar("test/ec_cls", test_ec_loss, epoch)
            writer.add_scalar("test/dice", test_dice, epoch)
            writer.add_scalar("test/acc_value", test_acc_value, epoch)
