...
def train_age(cfg, epoch, model, optimizer, labeled_train_loader, mixUp_dataset, use_cuda):
    global loss_value, losses
    model.train()
    criterion = cfg.loss.losses[0]
    ec_seg = nn.CrossEntropyLoss(weight=torch.tensor([0.01, 0.99])).cuda()
    ...
    for i, data in enumerate(labeled_train_loader):
        ...
        #     continue
        if cfg.train.mixup:
            x, mask, label, age, sex = data
            # pdb.set_trace()
            lam = np.random.beta(cfg.train.alpha, cfg.train.alpha)
            x2, mask2, label2, age2, sex2 = next(generater)
            if label.size()[0] != label2.size()[0]:
                break
            if cfg.net.seg_class == 2:
                label[label != 0] = 1
                label2[label2 != 0] = 1
            x = lam * x + (1 - lam) * x2
            age = lam * age + (1 - lam) * age2
            sex = lam * sex + (1 - lam) * sex2
            if use_cuda:
                x = x.cuda()
                label, label2 = label.cuda(), label2.cuda()
                age, sex = age.cuda(), sex.cuda()
                mask, mask2 = mask.cuda(), mask2.cuda()
            seg_pred, pred = model(x, age, sex)
            # import pdb
            # pdb.set_trace()
            loss_value1 = lam * criterion(pred, label) + (1 - lam) * criterion(pred, label2)
            loss_value2 = lam * ec_seg(seg_pred, mask) + (1 - lam) * ec_seg(seg_pred, mask2)
            ...
            # loss_value = loss_value1 + 0.1 * loss_value2 + 0.1 * loss_value3
            loss_value = loss_value1 + 0.0 * loss_value2 + 0.0 * loss_value3
            losses = [loss_value1.item(), loss_value2.item(), loss_value3.item()]
            # import pdb
            # pdb.set_trace()
            loss_1.update(loss_value1.item())
            ...
            # import pdb
            # pdb.set_trace()
            loss_value1 = criterion(pred, label)
            loss_value2 = ec_seg(seg_pred, mask)
            loss_value3 = dice_loss(seg_pred, mask)
            ...
            loss_3.update(loss_value3.item())
            sum_loss.update(loss_value.item())
            msg = "wo mixup epoch: {},  iter:{} / all:{},  total: {:.4f}, ec_loss_cls: {:.4f}, ec_loss_seg: {:.4f}, " \
                  "dice_loss_seg: {:.4f}".format(epoch, i, len(labeled_train_loader), sum_loss.avg,
                                                 loss_1.avg, loss_2.avg, loss_3.avg)
            print(msg)
        else:
...
def val_age(cfg, writer, epoch, model, val_dataset, use_cuda):
    # implemented
    model.eval()
    if use_cuda:
        model.cuda()
...
def val(cfg, writer, epoch, model, val_dataset, use_cuda):
    # implemented
    model.eval()
    if use_cuda:
        model.cuda()
...
def main(cfg):
    if cfg.net.name == "Unet_cls":
        model = unet.UnetPlusPlus_Cls(cfg.net.encoder_block, cfg.net.seg_class, cfg.net.cls_class, )
        ...
        try:
            ...
            pass
        train_cls_dataset = data_generator.cls_generator_2D(labeled_f['img'],
        ...
        val_cls_dataset = data_generator.cls_val_2D(labeled_f_val['img'],
                                                    labeled_f_val['mask'], val_labels,
                                                    rotate_degree=0,
                                                    noise_sigma=(0, 1e-9), )
        val_dataLoader = DataLoader(val_cls_dataset, batch_size=1, shuffle=False)
    elif cfg.net.name == "Unet_cls_age":
        model = unet.UnetPP_Cls_age(
            encoder_name=cfg.net.encoder_block, in_channels=1, classes=cfg.net.seg_class, aux_params=dict(
                pooling='avg',  # one of 'avg', 'max'
                dropout=0,  # dropout ratio, default is None
                activation=None,  # activation function, default is None
                classes=cfg.net.cls_class,  # define number of output labels
            ))
        # pdb.set_trace()
        labeled_f = h5py.File(cfg.data.train_txt, 'r')
        labeled_f_val = h5py.File(cfg.data.val_txt, 'r')
        ...
        train_cls_dataset = data_generator.cls_generator_2D_age(labeled_f['img'],
                                                                labeled_f['mask'], train_labels, labeled_f['age_info'],
                                                                labeled_f['sex_info'],
                                                                rotate_degree=cfg.data.random_rotate,
                                                                noise_sigma=cfg.data.random_Noise, )
        train_dataLoader = DataLoader(train_cls_dataset, batch_size=cfg.train.batch_size, shuffle=True, num_workers=16)
    ...
    for epoch in range(cfg.train.start_epoch + 1, cfg.train.epochs + 1):
        loss_sum, loss1, loss2, loss3 = train_age(cfg, epoch, model, optimizer, train_dataLoader, mixUp_dataset,
                                                  use_cuda)
        ...
        # with torch.no_grad():
        #     print("==> Validating! ")
        #     ec_loss, val_dice, acc_value = val_age(cfg, writer, epoch, model, val_dataLoader, use_cuda)
